<template>
  <Disclosure
    as="nav"
    class="relative z-20 border-b border-terminal-border bg-terminal-bg"
    v-slot="{ open, close }"
  >
    <div class="flex items-center justify-between px-6 py-5 md:px-12">
      ...
      <RouterLink
        to="/"
        class="text-sm font-bold tracking-[0.05em] text-terminal-accent"
      ...
      <div class="hidden items-center gap-8 md:flex">
        ...
        >
          ...
          >
            <template v-if="item.exact ? isExactActive : isActive"
              >&gt;&nbsp;</template
            >{{ $t(item.key) }}
          </a>
        </RouterLink>
        ...
        <LanguageSwitcher
          class="border-l border-terminal-border pl-5 text-xs text-terminal-muted"
        />
      </div>
    ...
    <DisclosurePanel class="md:hidden">
      <div
        class="flex flex-col gap-5 border-t border-terminal-border px-6 py-5"
      >
        <RouterLink
          v-for="item in navItems"
          :key="item.to"
        ...
        <LanguageSwitcher
          class="border-t border-terminal-border pt-4 text-xs text-terminal-muted"
        />
      </div>
    </DisclosurePanel>
  </Disclosure>
